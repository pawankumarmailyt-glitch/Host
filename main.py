...
def parse_data(data):
    try:
        # Different API structures handle karega
        if "data" in data:
            info = data["data"]
        elif "result" in data:
            info = data["result"]
        ...
        return {
            ...
            "father": info.get("father_name") or info.get("father") or info.get("fatherName") or "N/A",
...
def process_lookup(chat_id, number):
    try:
        ...
        print(data)  # 🔥 DEBUG (Render logs me check karo)
